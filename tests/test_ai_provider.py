...
# ── KiloProvider (Phase 10) ──────────────────────────────────────────────────
def test_factory_returns_kilo_provider(monkeypatch):
    from config import get_settings
    get_settings.cache_clear()
    monkeypatch.setenv("AI_PROVIDER", "kilo")
    monkeypatch.setenv("AI_API_KEY", "kilo-jwt-token")
    get_settings.cache_clear()
    p = get_provider()
    from ai_provider import KiloProvider
    assert isinstance(p, KiloProvider)
    assert p.name == "kilo"
    assert p.base_url == "https://api.kilo.ai/api/gateway"
    assert p.model == "anthropic/claude-3-5-sonnet"


def test_kilo_provider_accepts_custom_model(monkeypatch):
    from config import get_settings
    get_settings.cache_clear()
    monkeypatch.setenv("AI_PROVIDER", "kilo")
    monkeypatch.setenv("AI_API_KEY", "kilo-jwt")
