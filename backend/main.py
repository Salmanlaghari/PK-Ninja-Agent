...
@app.post("/api/config")
async def api_update_config(body: ConfigIn) -> dict:
    """Dynamically update the active AI provider configuration."""
    settings = get_settings()
    ...
    if body.base_url:
        settings.ai_base_url = body.base_url

    # Mirror into legacy gemini variables if needed
